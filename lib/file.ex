    ...
    * `ExMake.Utils`
    ...
    defmacro __using__(_) do
        quote do
            ...
            Module.register_attribute(__MODULE__, :exmake_cache, [persist: true])
    ...
    defmacro __before_compile__(_) do
        quote do
            def __exmake__(:cache), do: @exmake_cache
    ...
    @doc %B"""
    Indicates that ExMake is allowed to cache the dependency graph for this script
    file and those it recurses into.

    The persisted graph is saved to a `.exmake` directory alongside the script file.
    ...
    The graph for this script file is, the first time ExMake is invoked on it, saved
    into `.exmake` (which is created if it doesn't exist).

    The cached graph will be invalidated whenever any of the script files it was
    generated from have changed (not including any libraries those script files use).
    Note that you should not use this feature if the dependency graph generated by
    your script file(s) depends on external or non-constant factors (such as
    environment variables set from the shell), as ExMake then won't pick up the
    changed graph.
    """
    defmacro cache() do
        quote do: @exmake_cache true
    end
